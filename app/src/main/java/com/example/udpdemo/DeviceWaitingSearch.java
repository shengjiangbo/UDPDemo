...
public abstract class DeviceWaitingSearch extends Thread {
    ...
    public DeviceWaitingSearch(Context context, int number, String room) {
        mContext = context;
        deviceRoom = room;
        this.number = number;
    }
        ...
        try {
            ...
            while (true) {
                ...
                if (verifySearchData(pack)) {
                    ...
                    System.out.println( "@@@zjun: 给主机回复信息");
                    socket.send(sendPack);
                    System.out.println("@@@zjun: 等待主机接收确认");
                    socket.setSoTimeout(RECEIVE_TIME_OUT);
                    try {
                        socket.receive(pack);
                        if (verifyCheckData(pack)) {
                            System.out.println( "@@@zjun: 确认成功");
    ...
    private byte[] packData() {
        ...
        byte[] temp = getBytesFromType(PACKET_DATA_TYPE_DEVICE_NAME_20, number+"");
    ...
    private boolean verifyCheckData(DatagramPacket pack) {
        ...
        System.out.println("@@@zjun: ip from host=" + ip);
    ...
    private String getOwnWifiIP() {
        ...
        System.out.println("@@@zjun: 本机IP=" + ipAddr);
        return int2Ip(ipInt);
    }
