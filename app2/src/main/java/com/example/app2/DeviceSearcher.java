...
public abstract class DeviceSearcher extends Thread {
    ...
    private Set<VideoCreateBean> mDeviceSet;
    ...
    private void sendsSearchMessage(DatagramPacket sendPack) throws IOException {
        for (int i = 0; i < 3; i++) {
            // 发送搜索广播
                ...
                while (rspCount-- > 0) {
                    ...
                    if (recePack.getLength() > 0) {
                        ...
                        if (parsePack(recePack)) {
                            System.out.println("Demo: 设备上线：" + mDeviceIP);
            ...
            System.out.println("Demo: 结束搜索" + i);
        }
     ...
     */
    public abstract void onSearchFinish(Set<VideoCreateBean> deviceSet);
    ...
    private boolean parsePack(DatagramPacket pack) {
        ...
        for (VideoCreateBean d : mDeviceSet) {
            if (d.getHost().equals(ip)) {
//                return false;
                isExist = false;
            }
        }
        ...
        while (offset + 5 < dataLen) {
            ...
            switch (type) {
                case PACKET_DATA_TYPE_DEVICE_NAME_20:
                    ...
                    device = new VideoCreateBean();
                    device.setNumber(Integer.parseInt(name));
                    device.setHost(ip);
                    device.setPort(port);
                    break;
